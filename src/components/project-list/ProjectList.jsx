...
import $ from "jquery";
import { translations } from "../../enums/translitions";
...
export default function ProjectList({ lang }) {
  ...
  const handleFilterLink = (key, index) => {
    ...
    setTimeout(() => {
      ...
      let left =
          parseInt(
            projectsRef.current[index].parentElement.parentElement.offsetLeft
          ) + parseInt(calcWrapper),
  ...
  const fetchProjectCount = async () => {
    const { data, error } = await supabase.from("project-list").select("id");
  ...
  const fetchProjects = async (from, to) => {
    ...
    if (error) {
      setProjects(null);
      setProjectsCatchErr(
        "Projeler yüklenirken bir hata oluştu, lütfen daha sonra tekrar deneyiniz (FETCHERR-01)"
      );
      // console.log(error);
    }
    if (data) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (
        projectsRef?.current[0]?.classList.value.includes("active") &&
        !filterContainer.current.classList.value.includes("search")
      ) {
        if (
  ...
  useEffect(() => {
    ...
    const fetchProjectType = async () => {
      ...
      if (error) {
        // console.log(error);
        setProjectsCatchErr(
          "Proje kategorileri yüklenirken bir hata oluştu, lütfen daha sonra tekrar deneyiniz (FETCHERR-02)"
        );
      }
      if (data) {
          ...
          <div className="filter-active" ref={filterContainer}>
            <div className="filter-links">
              ...
              >
                {projectType?.map((type, index) => {
                  ...
                  // API'den gelen key
                  const projectTypeKey = type.projectType;

                  // Çevrilmiş metin (yoksa orijinali)
                  const translatedLabel =
                    translations[lang][projectTypeKey] ?? projectTypeKey;
                      ...
                      <SwiperSlide key={index}>
                        ...
                        >
                          <div
                            ...
                            onClick={() =>
                              handleFilterLink(
                                projectTypeQuery[index - 1].projectType,
                                index
                              )
                            }
                            ref={(el) => (projectsRef.current[index] = el)}
            ...
            <div className="search-input">
              ...
              >
                ...
                <path
                  fill="#228ED7"
                  d="M20.805 19.445l-2.712-2.703a9.123 9.123 0 10-1.35 1.351l2.702 2.712a.96.96 0 001.644-.68.96.96 0 00-.284-.68zM3.833 11.02a7.188 7.188 0 1114.375 0 7.188 7.188 0 01-14.375 0z"
                ></path>{" "}
              </svg>
              <input
                type="text"
                onInput={() => handleSearchInput(searchInput.current.value)}
                ref={searchInput}
                placeholder="Proje ismiyle hızlı arama yapabilirsiniz..."
              />
            </div>
