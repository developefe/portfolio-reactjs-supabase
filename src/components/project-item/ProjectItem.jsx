    ...
    return(
        <div className={`item-container ${project.projectType +' '+ projectClass}`}>
            <div className="project-item" style={{scale: 0}} ref={projectItem}>
                {project.projectStatus === 'Tamamlandı' ? <span className="status">{lang == 'tr' ? 'Tamamlandı' : 'Completed'}</span> : <span className="status warning">{lang == 'tr' ? 'Devam ediyor' : 'On-going'}</span>}
                <div className="top">
                    <p className="name">
                        {project.projectName}
                    </p>
                    <div className="links">
                        {project.projectDesign && (
                            <a href={project.projectDesign} className="link-item" target="_blank">
                                ...
                                <div className="tooltip">{lang == 'tr' ? 'Prototipi incele' : 'Prototype'}</div>
                            </a>
                        )}
                            ...
                            <a href={project.projectWebsite} className="link-item" target="_blank">
                                {project.projectStatus === 'Tamamlandı' ? (
                                    <>
                                        ...
                                        <div className="tooltip">{lang == 'tr' ? 'Canlı siteyi incele' : 'Live site'}</div>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <div className="tooltip">{lang == 'tr' ? 'Demo siteyi incele' : 'Demo Site'}</div>
                                    </>
                                )}
                            </a>
                        )}
